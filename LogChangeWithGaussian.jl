...
function predict(w, input, h; gaussianinput = 0, gaussianhidden = 0)
    hx, cx = h
    r, wr, wy, by = w
...
function loss(w, data, h, gaussianx, gaussianh)
    input, ygold = data
    yhat = predict(w, input, h, gaussianinput = gaussianx, gaussianhidden = gaussianh)
    1/2 * mean(abs2, yhat - ygold)
end

lossgrad = grad(loss)

function train(w, batches, h, gaussianinput, gaussianhidden)
    optim = optimizers(w, Adadelta)
    for data in batches
        gradamount = lossgrad(w, data, h, gaussianinput, gaussianhidden)
...
function main()
    ...
    for i = 1:epocs
        h = hiddeninit(w)
        train(w, trn, h, gaussianinput, gaussianhidden)
