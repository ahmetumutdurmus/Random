...
function weightsinit(inputs; p = 0)
    hiddensize = ceil(Int, 50 / (1 - p))
    winit = 0.1
    layernum = 1
    rnntype = :gru
...
function predict(w, input, h; dropoutinput = 0, dropouthidden = 0)
    hx, cx = h
    r, wr, wy, by = w
...
function loss(w, data, h, dropoutx, dropouth)
    input, ygold = data
    yhat = predict(w, input, h, dropoutinput = dropoutx, dropouthidden = dropouth)
...
function main()
    ...
    w = weightsinit(inputs, dropouthidden)
    ...
    for i = 1:epocs
        ...
        tstDMstat = report(w, trn, tst, i, means, stddevs)
        if tstDMstat > 1.96
            info("significance achieved")
            break
        end
    end
end
